...
public class CrazyOptionals {
    ...
    public static Account getAccountWithMaxBalance(List<Account> accounts) {
        return accounts.stream().max((a1, a2) -> a1.getBalance().compareTo(a2.getBalance())).orElseThrow(NoSuchElementException::new);
    }
    ...
    public static void processAccountWithMaxBalance(List<Account> accounts, AccountService accountService) {
        accounts.stream().max((a1, a2) -> a1.getBalance().compareTo(a2.getBalance())).ifPresent(accountService::processAccount);
    }
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream().mapToDouble(a -> a.getCreditBalance().orElse(BigDecimal.ZERO).doubleValue()).sum();
    }
}
